...
from .quant_utils import quant_args, quant_args_low
...
def quantize_model_low(model):
    ...
    # quantize layers
    if type(model) == nn.Conv2d:
        quant_mod = Conv2dQuantizer(**quant_args_low)
        quant_mod.set_param(model)
        return quant_mod
    elif type(model) == nn.Linear:
        quant_mod = LinearQuantizer(**quant_args_low)
        quant_mod.set_param(model)
        return quant_mod
    elif type(model) == pytorch_utils.Conv1D:
        quant_mod = Conv1dQuantizer(**quant_args_low)
        quant_mod.set_param(model)
        return quant_mod
    elif type(model) == nn.Sequential:
        mods = []
        for n, m in model.named_children():
            mods.append(quantize_model_low(m))
        return nn.Sequential(*mods)
    elif type(model) == nn.ModuleList:
        mods = []
        for n, m in model.named_children():
        ...
        for attr in dir(model):
            mod = getattr(model, attr)
            if isinstance(mod, nn.Module) and attr != 'base_model' and attr!= 'lm_head':
                setattr(q_model, attr, quantize_model_low(mod))
        return q_model
