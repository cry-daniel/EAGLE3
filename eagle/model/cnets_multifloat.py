...
class Model(nn.Module):
    ...
    @torch.no_grad()
    def topK_genrate_w_quant(self, hidden_states, input_ids, head, logits_processor):
        ...
        # Condition 1: 同一个 branch 上的 score 做连乘作为权重
        # tree_branch_sum = torch.sum(draft_scores[retrieve_indices], dim=1)

        # Condition 2: 按照 bfs 的顺序判定重要性，值越大越重要
        all_branches = draft_scores[retrieve_indices]
        def bfssort(tensor):
            for d in range(1, tensor.shape[-1]):
                max_ = torch.max(tensor[:, d])
                tensor = tensor[torch.where(max_ == tensor[:, d])]
            return tensor[0,:]
        best_branch = bfssort(all_branches)
        # import pdb; pdb.set_trace()
        where_best_branch = torch.where((all_branches == best_branch).all(dim=1))[0]
        tree_branch_sum = torch.zeros(all_branches.shape[0], dtype=where_best_branch.dtype, device=where_best_branch.device)
        tree_branch_sum[where_best_branch] = 1
        ...
        return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, tree_branch_sum
