...
def initialize_tree_w_prune(input_ids, model, past_key_values, logits_processor):
    outputs, orig, hidden_states = model(
        input_ids, past_key_values=past_key_values, output_orig=True
    )
...
@torch.no_grad()
def update_inference_inputs_w_prune(
        input_ids,
        candidates,
        best_candidate,
        accept_length,
    ...
    if logits_processor is not None:
        ...
        token = token[None, None]
    # hidden_state = torch.cat((hidden_state, accept_hidden_state_new), dim=1)
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids, attn_output = model.ea_layer.topK_genrate_w_prune(accept_hidden_state_new,
                                              input_ids=torch.cat((input_ids, token.to(input_ids.device)), dim=1),
                                              head=model.base_model.lm_head,logits_processor=logits_processor)
